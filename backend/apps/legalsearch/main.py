...
class LegalQuery(BaseModel):
    query: str
    results_limit: int = Query(default=8, ge=1, le=10, description="Number of results to be shown (Max. 10)")
...
async def legal_search(query_data: LegalQuery):
    print(f"Query data received: {query_data.query}, Results Limit: {query_data.results_limit}")
    ...
    results = collection.query(query_texts=[query_data.query], n_results=query_data.results_limit)
    print(f"Documents returned: {len(results['documents'][0])}")  
    ...
    return jsonable_encoder({"answer": answer_text, "documents": documents})
